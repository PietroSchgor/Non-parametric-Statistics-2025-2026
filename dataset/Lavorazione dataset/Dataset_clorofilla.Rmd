...
nome_file <- "cmems_mod_med_bgc-plankton_my_4.2km_P1D-m_1770738700917.nc"
r <- rast(nome_file)
...
# Filtriamo escludendo (con il punto esclamativo !) la combinazione indesiderata
dataset_statistico_clorofilla_3anni_cut <- dataset_statistico_clorofilla %>%
  filter(!(Lon > 13 & Lat < 45.6))
```

```{r plot after-cut}
# 1. Calcola la media per ogni pixel (Lat/Lon) su tutto l'anno
mappa_media <- dataset_statistico_clorofilla_3anni_cut %>%
  group_by(Lon, Lat) %>%
  summarise(Mean_Chl = mean(Chl, na.rm = TRUE))
...
```{r write.csv}
write.csv(dataset_statistico_clorofilla_3anni_cut, "dataset_clorofilla_filtrata_3anni.csv", row.names = FALSE)
```